...
### 🔒 스레드 안전성 강화
- **전체 자막 접근 메서드 락 적용** - 17개 메서드에 `subtitle_lock` 추가
- **비동기 파일 저장** - TXT, SRT, VTT 저장 시 자막 수집 중단 없음
- **예외 처리 강화** - `_save_session`, `SubtitleEntry.from_dict` 등

### 🚀 성능 개선
- **헤드리스 모드 포트 동적 할당** - 다중 인스턴스 충돌 방지
- **타이머 상수 통일** - `Config.FINALIZE_CHECK_INTERVAL` 사용
