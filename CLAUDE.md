...
│  │  - event-driven hybrid probe + keepalive             │   │
│  │  - 자동 재연결 (지수 백오프, #31)                     │   │
│  │  - stop_event 기반 안전 종료                          │   │
│  └──────────────────────────────────────────────────────┘   │
│                            │                                 │
...
- 시작/재연결 시 URL에 `xcgcd`가 없으면 Worker에서 `xcode` 기준 자동 감지를 1회 수행해 보완 URL로 재접속한다.
- Observer 주입은 타겟 기반 우선 시도 후 실패 시 JS 폴링 브리지(`allow_poll_fallback`)로 전환한다.
- Observer active 상태에서는 매 tick full probe를 반복하지 않고, `observer change`, `keepalive`, `1.0초 health probe`, reconnect 시점에만 structured probe를 수행한다.
- frame path cache / active selector cache를 재사용하고, 큰 JS probe 본문은 frame별 1회 bridge 주입 후 짧은 호출만 반복한다.
- Worker preview 기본 계약은 `StructuredPreviewPayload`이며, UI는 legacy dict 수신도 호환한다.
...
- 세션 저장/자동 백업은 `snapshot_clone()` + streaming JSON writer를 사용한다.
...
## 6. 최신 변경 요약 (v16.14.2 기준)

### v16.14.2 성능 최적화 중심 리팩토링 메모
- **Worker CPU 절감**: observer idle 시 full probe를 `1.0초 health probe` cadence로 제한하고, selector refresh(`5초`) / frame inventory refresh(`10초`)를 분리
- **브리지/캐시 재사용**: frame path cache, active selector cache, frame별 JS probe bridge 재사용으로 브라우저 쪽 반복 비용 축소
