...
- `ui/main_window_impl/persistence_runtime.py`/`core/database_manager.py`/`core/subtitle_pipeline.py`/`subtitle_extractor.spec`: 공개 facade와 기존 테스트 진입점은 유지하면서 runtime persistence 세부 mixin(`persistence_runtime_*`), DB mixin(`core/database_impl/`), pipeline helper(`core/subtitle_pipeline_impl/`)로 보존형 분리를 추가했고, PyInstaller hidden import도 새 내부 패키지까지 확장했다. 이는 파일 크기와 책임 경계를 정리한 변경이며 글로벌 히스토리 + suffix 의미론은 바뀌지 않는다.
...
- `core/config.py`/`core/committee_catalog.py`/`core/live_list.py`: 2026-09-17 사이트 실측에서 청문회/공청회 live_list xcode가 `97`(구 `99`)이고 기후노동위 공식명이 `기후에너지환경노동위원회`로 확인되었다. 기본 프리셋은 현재 live_list에 맞추고, 구 xcode/구 공식명은 식별 카탈로그로 유지한다. `select_live_broadcast_row`는 정확 xcode가 없을 때만 위원회 식별자 1건 보정하며, `target_xcode` 없는 자동 선택은 하지 않는다. 네트워크/메타데이터 정합 변경이며 글로벌 히스토리 + suffix 의미론은 바꾸지 않는다.
- `core/config.py`/`core/live_list.py`/`ui/dialogs.py`/`ui/main_window_impl/capture_live.py`/`ui/main_window_impl/pipeline_messages.py`: v16.14.7(2026-05-18)에서 기본 URL/본회의 프리셋은 `xcode=10`으로 고정되고, `특별위원회(91)`, `청문회/공청회(99)` 기본 프리셋과 약칭이 추가되었다. stale `IO` 기본 프리셋은 제거되며, `target_xcode`가 없으면 단일 live row도 자동 선택하지 않는다. 이는 의도하지 않은 위원회로 URL이 보정되는 운영 리스크를 줄이는 네트워크/메타데이터 정책 변경이며 코어 자막 증분 파이프라인 의미론은 바꾸지 않는다.
- `ui/main_window_impl/persistence_runtime_archive.py`/`persistence_runtime_manifest.py`/`persistence_runtime_segments.py`: v16.14.7(2026-05-18)에서 runtime segment/tail load 시 `entry_count`, `first_entry_id`, `last_entry_id`, `entries_digest`를 실제 entries fingerprint와 대조한다. strict load는 실패하고 salvage는 해당 파일을 제외하며 `무결성 불일치` 경고를 남긴다. 이 변경은 복구 입력의 무결성 검증을 추가한 것이며 subtitle merge/history/suffix 의미론은 유지된다.
- `국회의사중계 자막.py`/`scripts/check_live_list_drift.py`/`scripts/run_release_verification.py`/`core/file_io.py`/`ui/main_window_impl/capture_live.py`/`ui/main_window_impl/persistence_session.py`: v16.14.7(2026-05-21)에서 `MainWindow()` constructor smoke, live-list name drift 리포트, live-list 실패 원인 status/toast 노출, 세션/백업 파일명 충돌 방지, release verifier 옵션화가 추가되었다. 이는 운영 진단·검증·저장 파일명 hygiene 변경이며 글로벌 히스토리 + suffix 의미론은 바꾸지 않는다.
- `core/url_policy.py`/`ui/main_window_impl/runtime_lifecycle.py`/`ui/main_window_impl/ui/history_presets.py`/`persistence_runtime_manifest.py`/`core/database_impl/search_stats.py`/`database_worker.py`: v16.14.7(2026-06-04)에서 시작 URL/프리셋/URL history sanitize 정책을 `assembly.webcast.go.kr` 계열로 공통화하고, runtime manifest malformed segment strict/salvage 처리와 DB 검색 실패 표시를 보강했다. 이는 입력/복구/DB 오류 가시성 hardening이며 글로벌 히스토리 + suffix 의미론은 바꾸지 않는다.
...
- `ui/main_window_impl/pipeline_state.py`/`persistence_exports.py`/`persistence_session.py`/`persistence_runtime.py`/`persistence_tools.py`/`pipeline_queue.py`/`pipeline_messages.py`/`core/database_manager.py`/`국회의사중계 자막.py`: v16.14.7(2026-05-06)에서 저장/export/자동백업/리플로우 worker는 clone 기반 persistent snapshot을 사용하고, `finished`/`error`/`subtitle_not_found` terminal worker message는 bounded queue 포화 시 priority passthrough로 보존한다. FTS5 rebuild는 최초 생성 또는 sample probe drift 등 필요 시에만 수행하며, source/frozen smoke CLI가 추가되었다. 이는 저장·큐·DB·검증 hardening이며 글로벌 히스토리 + suffix 의미론 자체는 바꾸지 않는다.
- `ui/main_window_impl/capture_browser.py`/`pipeline_messages.py`/`capture_observer.py`: v16.14.8(2026-07-22)에서 ExtractionWorker `finally`의 `finished`를 `clear_worker_run_id` 이전 `_emit_worker_message(..., run_id=)` terminal 경로로 고정하고, stop 중 `finished`/`error`를 멱등 흡수하며, Observer JS 짧은 발화 필터를 파이프라인(`is_meaningful_subtitle_text`)과 정렬했다. 큐/종료/수집 입력 게이트 hardening이며 글로벌 히스토리 + suffix 코어 의미론은 바꾸지 않는다.
- `ui/main_window_impl/pipeline_stream.py` `_soft_resync`: v16.14.8(2026-07-29)에서 최근 5개 활성 엔트리 기반 재구성을 유지하되, 기존 `_confirmed_compact`가 최근 창/마지막 엔트리와 정합하면(포함·접미·말단 스캔) **더 긴 compact 히스토리를 유지**한다. runtime archive 이후 active tail만 남은 세션에서 suffix 맥락 축소를 완화한다. `rfind` suffix 추출 의미론과 `_process_raw_text` 코어는 변경하지 않는다.
- `ui/main_window_impl/runtime_lifecycle.py` `_start`/`_begin_extraction_run`: v16.14.8(2026-07-29)에서 dirty 세션은 종료·불러오기와 동일한 저장/버리기/취소 흐름을 거치고, clean 이지만 자막이 있으면 교체 확인 후 시작한다. 세션 UX hardening이며 수집 알고리즘 의미론은 바꾸지 않는다.
- `ui/main_window_impl/persistence_runtime_segments.py`: fingerprint 불일치 시 manifest 미등록 orphan `segment_*.json`을 best-effort 삭제한다. 복구 I/O hygiene이며 코어 추출 의미론은 바꾸지 않는다.
...
- `core/models.py`/`core/config.py`: `SubtitleEntry(entry_id=None)` 자동 ID 생성, `snapshot_clone()` 문서화, 미검증 `정보위원회`/`NA`/`PP`와 stale `IO` 기본 프리셋 제거, live-list 기준 `본회의(10)`, `특별위원회(91)`, `청문회/공청회`(당시 99, 2026-09 이후 97) 유지
...
- 2026-05-11 보존형 분리에서는 `persistence_runtime.py`, `database_manager.py`, `subtitle_pipeline.py`도 facade/API 표면을 유지하면서 `ui/main_window_impl/persistence_runtime_*`, `core/database_impl/`, `core/subtitle_pipeline_impl/`로 내부 구현을 더 나눴다.
