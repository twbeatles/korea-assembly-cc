...
- `core/models.py`/`core/config.py`: `SubtitleEntry(entry_id=None)` 자동 ID 생성, `snapshot_clone()` 문서화, 미검증 `정보위원회`/`NA`/`PP`와 stale `IO` 기본 프리셋 제거, live-list 기준 `본회의(10)`, `특별위원회(91)`, `청문회/공청회(99)` 유지
